...
import { webMessagesFromRecords } from "../src/web/messages.js";
...
describe("web voice call", () => {
	...
	it("marks a discarded call for you alone", async (t) => {
		const config = await configWithDataDir(t, await createTempDataDir(t));
		const records: ChatLogRecord[] = [
			...chat(),
			{ type: "call_discarded", ...buildRecordBase(ownerDmRef, 4), durationMs: 252_000 },
		];
