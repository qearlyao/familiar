...
import { Card, Field, MODEL_REF, OnOffToggle, TextInput } from "./inputs";
  ...
  return (
    <Card title="pictures" off={enabled !== true} action={<OnOffToggle enabled={enabled} disabled={disabled} ariaPrefix="image generation" onChange={(next) => void onChange("image_gen.enabled", next)} />}>
      <Field label="primary · provider/model-id">
        <TextInput pattern={MODEL_REF} value={values?.["image_gen.model"].value} placeholder="openrouter/google/gemini-2.5-flash-image" allowEmpty={false} disabled={fieldsDisabled} onCommit={(next) => onChange("image_gen.model", next)} />
      </Field>
      <Field label="fallback · empty for none">
        <TextInput pattern={MODEL_REF} value={values?.["image_gen.fallback_model"].value} placeholder="none" allowEmpty disabled={fieldsDisabled} onCommit={(next) => onChange("image_gen.fallback_model", next)} />
      </Field>
    </Card>
  );
}
