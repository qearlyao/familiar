...
- Add optional ElevenLabs voice settings as model-aware config, not one flat preset: v2/v2.5 support `stability`, `similarity_boost`, `style`, `speed`, and `use_speaker_boost`; v3 supports audio tags and does not support Speaker Boost. Keep defaults conservative and omit unsupported fields per `model_id`.
